...
class CentreonGraph
{
    ...
    public function getOVDColor($metricId)
    {
        if (is_null($this->colorCache)) {
            ...
            $DBRESULT = $this->DB->query("SELECT metric_id, rnd_color FROM `ods_view_details` " .
                " WHERE `index_id` = " . $this->index);
        ...
        $stmt = $this->DB->prepare("INSERT INTO `ods_view_details` (rnd_color, index_id, metric_id) VALUES ('" .
            $lRndcolor . "', " . $this->index . ", :metricId)");
        $stmt->bindValue(':metricId', $metricId, PDO::PARAM_STR);
        $stmt->execute();
        return $lRndcolor;
    }
